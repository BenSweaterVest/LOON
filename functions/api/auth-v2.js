/**
 * ============================================================================
 * LOON Auth-v2 Endpoint (functions/api/auth-v2.js)
 * ============================================================================
 * 
 * Phase 2 (Team Mode) authentication using Cloudflare KV for user storage
 * and session tokens for authentication.
 * 
 * WHY PHASE 2?
 * Phase 1 uses one password per page (stored in env vars), limiting you to
 * ~95 users. Phase 2 stores users in Cloudflare KV, supporting unlimited
 * users with role-based access control (RBAC).
 * 
 * ENDPOINTS:
 *   GET    /api/auth-v2 - Verify session token, get session info
 *   POST   /api/auth-v2 - Login and receive session token
 *   PATCH  /api/auth-v2 - Change own password (authenticated)
 *   DELETE /api/auth-v2 - Logout and invalidate session
 * 
